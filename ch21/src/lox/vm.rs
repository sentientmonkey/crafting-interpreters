...
mod tests {
    ...
    fn assert_interpret(source: &str, expected_value: &str) -> Rc<VM> {
        let statement = format!("print {};", source);
        let mut vm = VM::new();
        let result = vm.interpret(statement.as_str()).expect("failed");
        let expected_with_newline = format!("{}\n", expected_value);
        assert_eq!(
            expected_with_newline.as_str(),
            &result,
            "failed with: {}",
            source
        );
    ...
    fn it_negates() {
        ...
        chunk.write_chunk(OpCode::Print, 123);
        chunk.write_chunk(OpCode::Return, 123);

        let mut vm = VM::new();
        vm.chunk = chunk;

        assert_eq!("-1.2\n", vm.run().expect("failed"));
    }
    ...
    #[test]
    fn it_can_do_arthmetic() {
        assert_interpret("1 + 2", "3");
        assert_interpret("3 * 4", "12");
    }
