...
class EnvironmentTest < Minitest::Test
  ...
  def test_can_define_value
    @environment.define(@token.lexeme, 42)

    assert_equal 42, @environment.get(@token)
  end

  def test_can_assign_value
    @environment.define(@token.lexeme, 0)
  ...
  def test_enclosed_environment
    ...
    @environment.define(@token.lexeme, 21)
    enclosed_environment.define(other_token.lexeme, 50)
    enclosed_environment.define(@token.lexeme, 42)
