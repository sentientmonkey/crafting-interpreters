...
impl Parser {
    ...
    fn number(&mut self) -> Result<(), ParserError> {
        match self.previous.clone().unwrap().token_type {
            TokenType::Number(n) => self.emit_constant(n),
...
mod tests {
    ...
    fn it_compiles_binary() {
        ...
        assert_same(chunk.constants, vec![1.0, 2.0]);
    }
    ...
    fn it_compiles_with_precedence() {
        ...
        assert_same(chunk.constants, vec![2.0, 3.0, 4.0]);
    }
    ...
    fn it_compiles_strange() {
        ...
        assert_same(chunk.constants, vec![1.0, 2.0, 3.0, 4.0]);
    }
}
