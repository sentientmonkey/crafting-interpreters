...
class ParserTest < Minitest::Test
  ...
  def test_class
    ...
    ast = '(class Breakfast (def (cook) (print "Eggs a-frying\'!")) (def (serve who) (print (+ (+ "Enjoy your breakfast, " who) "."))))'

    assert_ast ast, code
  end

  def test_class_property_get
    assert_ast '((egg (scramble 3.0)) (with "name"))', 'egg.scramble(3).with("name");'
  end

  def test_class_property_set
    assert_ast '(someObject (set someProperty value))', 'someObject.someProperty = value;'
  end

  def test_this_keyword
    assert_ast '(print this)', 'print this;' 
  end
  ...
  def test_inheritence
    ...
    assert_ast expected, code
  end

  def test_super
    assert_ast '(super cook)', 'super.cook();'
  end
end
